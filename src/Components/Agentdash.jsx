...
const Agentdash = (props) => {
  ...
  const toastSuccess = (i) => {
   ...
   ;
    ...
    axios
      .post(
        `http://ec2-52-66-237-19.ap-south-1.compute.amazonaws.com:8001/orderapproved`,
        {
          order_id: b,
  ...
  useEffect(() => {
    ...
    setSrc(localStorage.getItem("source_info"))

  }, [])
  // #when clicked reject button post call and goes to reject screen
  const toastError = (i) => {
    let a = [...data1];
    let b = a[i].order_id;
    // props.dis(b);
  ...
  useEffect(() => {
    ...
    axios
      .get(
        "http://ec2-52-66-237-19.ap-south-1.compute.amazonaws.com:8001/successorder",
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("Token")}` }
        }
      )
      .then((response) => {
        ...
        localStorage.setItem("source_info", response.data.source_info)
      })
      .catch((error) => {
        console.log(error);
      });
                ...
                <table className="table">
                  ...
                  <tbody>
                    {data1.map((v, i) => {
                      {
                        display &&
                          localStorage.getItem("ORDERID" == v.order_id);
                        {
                          return (
                            <tr key={i}>
                              <td className="tbname">{v.order_id}</td>
                              <td className="tbname">{v.date_info}</td>
                              <td className="tbname">
                                {v.order_type}
                              </td>
                              ...
                              <td className="tbname">{v.qty}</td>
                              <td>
                                <div>
                                  <button
                                    type="button"
                                    onClick={() => toastSuccess(i)}
      ...
      <div className="row">
        ...
        <div className="col-4">
          <div className="agentside">
            ...
            <p className="repo1 mb-4">
              {" "}
              &#8594; If any occurance happen during ordering of a item can
              directly report to{" "}
              <span style={{ color: "blue", textDecoration: "underline" }}>
                onlinedeliverygoods@gmail.com
              </span>
            </p>
            <p className="repo1 mb-4">
              {" "}
              &#8594; Doing suspicious activities in the website can leads to
              block the user account permanently blocked
            </p>
            <p className="repo1 mb-4">
              {" "}
              &#8594; During exploring of the website if any bug occurs can
              report to the respective mail will be rewarded.
            </p>
            <p className="repo1 mb-4">
              {" "}
              &#8594; This website is user-friendly if any third party
              application causes we will avoid the use of login
            </p>
            <p className="repo1 mb-5">
              {" "}
              &#8594; Can report whatever you feels of website, we are happy to
              find a solution.
            </p>
            <div className="center mb-3">
              <button className="btn btn-danger" onClick={closehandle}>
                Close
              </button>
            </div>
          </div>
          ...
          <div className="agentside">
            ...
            <div className="row">
              <div className="col-6 mb-3">
                ...
                <p className="repo1 m-3">
                  &#8594; Agent should aware of orders.
                </p>
                <p className="repo1 m-3">
                  &#8594; Agent should delivery with in the time
                </p>
                <p className="repo1 m-3">
                  &#8594; Agent should aware of orders.
                </p>
                <p className="repo1 m-3">
                  &#8594; Behave properly with customers
                </p>
                <p className="repo1 m-3">
                  &#8594; Must be sanitized and dressed properly
                </p>
              </div>
              <div className="col-6">
                <center>
                ...
                <p className="repo1 m-3">
                  &#8594; Agent should not speak non-verbal language
                </p>
                <p className="repo1 m-3">
                  &#8594; Without informing the receiver and order delivery
                  should be avoided
                </p>
                <p className="repo1 m-3">
                  &#8594; Don't be rude with customers while delivering.
                </p>
                <p className="repo1 m-3">
                  &#8594; Avoid of doing scams while delivering the orders.
                </p>
              </div>
            </div>
            <center>
